...
export const useAttendance = () => {
  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      // Use employeeId (references employee_onboarding.id), not userId
      const employeeId = user.employeeId || user.employee_id || user.id || 2;
      
      console.log('👤 Retrieved user object:', user);
      console.log('🔑 Using employeeId (employee_onboarding.id):', employeeId);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data) {
            ...
            // The API already returns all records including today's, so just use that
